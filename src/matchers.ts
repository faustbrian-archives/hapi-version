...
export function matchAcceptHeader(request: Hapi.Request) {
	...
	const pattern = new RegExp(
		`^${standardsTree}.[a-z][a-z0-9.!#$&^_-]{0,126}.v[0-9]+$`,
		"i"
	);
	...
	if (media.isValid()) {
		...
		if (pattern.test(media.subtype)) {
			const vendorFacets = media.subtypeFacets.slice(
				1,
				media.subtypeFacets.length - 1
			);
			const vendorName = vendorFacets.join(".");

			if (vendorName === subType) {
				return caster(
					media.subtypeFacets[media.subtypeFacets.length - 1].slice(1)
				);
			}
		}
		...
		const versionParam =
			media.parameters[config.get("strategies.acceptHeader.parameter")];
